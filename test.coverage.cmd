...
set ADDTL_TESTPARAMS=--no-restore
...
for /F "tokens=* usebackq" %%p in (`dir /b /ad /s %BASE%\%TESTPROJECTS_PATTERN%`) do (
    dotnet test --project %%p ^
        --coverlet ^
        --results-directory %TEMPRESULTS_DIR% ^
        %ADDTL_TESTPARAMS%
)

rem Run the individual coverage reports through the reportgenerator tool to combine and generate HTML output.
reportgenerator -reports:%TEMPRESULTS_DIR%\**\coverage.cobertura*.xml ^
    -reporttypes:Html;Cobertura ^
    -targetdir:%COVERAGE_DIR% ^
    -assemblyfilters:-Moq;-StructuredFieldValues;-%TESTPROJECTS_PATTERN% ^
    %ADDTL_GENERATORPARAMS% ^
    -verbosity:Warning
